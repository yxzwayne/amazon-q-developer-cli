...
When set to `true` (default), the agent will have access to all MCP servers defined in the global configuration in addition to those defined in the agent's `mcpServers` field.
